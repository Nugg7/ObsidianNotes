...
    serv.Description,
    serva.Quantity,
    RANK() OVER(PARTITION BY serv.ID_Service, EXTRACT(YEAR FROM serva.ValidDate),
      EXTRACT(MONTH
      FROM
        serva.ValidDate)
...
    EXTRACT(YEAR
    FROM
      serva.ValidDate) AS YEAR_BILLED,

    EXTRACT(MONTH
    FROM
      serva.ValidDate) AS MONTH_BILLED,

  FROM
    `sigemi-prod.xtsDATA_sigemi.dbo_vw_SERV_Service` serv
  LEFT JOIN
    sigemi-prod.xtsDATA_sigemi.dbo_SERV_Service_ActualData AS serva
  ON
    serv.ID_Service = serva.ID_Service

  WHERE
    FeeType IN (1,
      2,
      3)
    AND serva.IsValid = TRUE
