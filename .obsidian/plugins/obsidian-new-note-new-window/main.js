...
var NewNoteNewWindow = class extends import_obsidian3.Plugin {
  constructor() {
    super(...arguments);
    this.leaves = [];
  }
  async onload() {
    ...
    const trackClose = (leafStatus) => {
      this.registerInterval(window.setInterval(() => {
        if (leafStatus.status == "loaded" && leafStatus.leaf.getViewState().type == "empty") {
          this.leaves = this.leaves.filter((l) => l.leaf != leafStatus.leaf);
          leafStatus.leaf.detach();
        }
      }, 10));
    };
    const createLeaf = (where) => {
      let leaf = void 0;
      ...
      const newLeafStatus = { leaf, status: "new" };
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        let nleafStatus = this.settings.openInLastFloatingWindow ? createLeaf("same-window") : createLeaf("new-window");
  ...
  getFileParent() {
    if (!this.settings.useDefaultFolder) {
      ...
      if (abstractFile && "children" in abstractFile) {
        ...
        new import_obsidian3.Notice(`Error opening folder '${folder}'!`);
  ...
  generateNewFileNameInFolder() {
    const tfolder = this.getFileParent();
    let newFilePath = tfolder.path;
    let untitleds = tfolder.children.filter((c) => c.name.startsWith("Untitled")).map((c) => c.name);
    let fileName = "";
...
var SettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Reuse the same floating window.").setDesc("If true new notes will open in the same floating window.").addToggle((cb) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Create in the default folder.").setDesc("Create the new files in the default folder as per Obsidian's configuration.").addToggle((cb) => {
    ...
    const folderSetting = new import_obsidian3.Setting(this.containerEl).setName("Custom folder").setDesc("Custom folder in wich to create the new notes.").addSearch((cb) => {
      new FolderSuggest(cb.inputEl);
      cb.setPlaceholder("Example: folder1/folder2").setValue(this.plugin.settings.customFolder).onChange((new_folder) => {
        this.plugin.settings.customFolder = new_folder;
        this.plugin.saveSettings();
      });
    });
    folderSetting.settingEl.style.display = this.plugin.settings.useDefaultFolder ? "none" : "block";
